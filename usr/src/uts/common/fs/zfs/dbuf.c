...
retry:
	...
	dbuf_zero_abd = abd_alloc_for_io(SPA_MAXBLOCKSIZE, B_FALSE);
	abd_zero(dbuf_zero_abd, SPA_MAXBLOCKSIZE);
...
dbuf_sync_leaf(dbuf_dirty_record_t *dr, dmu_tx_t *tx)
{
	...
	if (db->db_state == DB_UNCACHED) {
		...
		ASSERT(db->db_state == DB_CACHED || db->db_state == DB_ZEROFILL);
	}
	DBUF_VERIFY(db);
...
/* XXX why needed? */
static void
dbuf_write_zerofill_ready(zio_t *zio)
{
	dbuf_write_ready(zio, NULL, zio->io_private);
}

static void
dbuf_write_zerofill_done(zio_t *zio)
{
...
dbuf_write(dbuf_dirty_record_t *dr, arc_buf_t *data, dmu_tx_t *tx)
{
	...
	if (db->db_level == 0 &&
		...
		dr->dr_zio = zio_write(zio, os->os_spa, txg,
		    &dr->dr_bp_copy, dbuf_zero_abd, db->db.db_size, db->db.db_size, &zp,
